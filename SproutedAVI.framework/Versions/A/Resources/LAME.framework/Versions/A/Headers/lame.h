 ...
 * API calls.
 */
lame_global_flags * CDECL lame_init(void) WEAK_LINK;
/* obsolete version */
...
/*
	OPTIONAL: lame_fopen()
	You may use native file system calls to open, write and close your mp3 file.
	However, if you do, you will not be able to use the lame_mp3_tags_fid() call to 
	write a Xing header.  
	If you use the FILE API's directly, they *may* be compatible with lame_mp3_tags_fid(),
	however this is not guaranteed, because the library compiler may have linked against
	a different implementation.  eg: on Mac OS, if the library is compiled with XCode and the
	application is compiled with CodeWarrior, there will be utterly separate implementations
	of the FILE API's.  In that case, if you wish to use lame_mp3_tags_fid(), you *must* 
	use these provided LAME file calls, which are guaranteed to link against the same
	FILE library that lame_mp3_tags_fid() links against.
*/
#define	kLAME_Default_fopen_permissions		"wb+"
FILE* 	CDECL	lame_fopen(const char *pathZ, const char *permZ) WEAK_LINK;
size_t	CDECL	lame_fwrite(const void *bufZ, size_t itemSizeL, size_t itemsL, FILE *fP);
int		CDECL	lame_fclose(FILE *fP);
 ...
 * WARNING: see notes above on lame_fopen()
 */
