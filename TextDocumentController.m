...
- (id) init 
{
	...
	{
		lastScale = 100;
		[NSBundle loadNibNamed:@"TextDocumentView" owner:self];
	}
	return self;
}
...
- (BOOL) loadURL:(NSURL*)aURL 
{
	...
	// load depends on uti of file: kUTTypeRTF, kUTTypeRTFD, kUTTypePlainText
	...
	if ( UTTypeConformsTo( (CFStringRef)fileUTI, kUTTypeRTF ) )
	{
		attributedContent = [[[NSAttributedString alloc] initWithRTF:[NSData dataWithContentsOfFile:path] documentAttributes:nil] autorelease];
	}
	
	else if ( UTTypeConformsTo( (CFStringRef)fileUTI, kUTTypeRTFD ) )
	{
	...
	else if ( UTTypeConformsTo( (CFStringRef)fileUTI, kUTTypePlainText ) )
	{
		NSString *plainText = [NSString stringWithContentsOfFile:path usedEncoding:NULL error:nil];
		...
		if ( plainText == nil )
		{
			...
			for ( i = 0; i < 2; i++ )
			{
				plainText = [NSString stringWithContentsOfFile:path encoding:encodings[i] error:&error];
				if ( plainText != nil )
					break;
			}
		}
		...
		if ( plainText != nil )
			attributedContent = [[[NSAttributedString alloc] initWithString:plainText attributes:nil] autorelease];
	}
	...
	if ( attributedContent == nil )
	{
		...
		else [[NSAlert mediaError] runModal];
...
{
	return BundledImageWithName(@"TextEditBarSmall.png", @"com.sprouted.interface");
}
